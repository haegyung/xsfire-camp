...
| 항목 | 설명 |
| - | - |
| 바이너리 설치 | `scripts/build_and_install.sh`로 `codex-acp`를 `$HOME/.local/bin` 등에 복사하세요. |
| 환경 변수 | `CODEX_HOME`과 `PATH`에 설치 위치를 명시해야 CLI와 ACP가 동일한 홈을 사용합니다. |
| Zed 등록 | `settings.json`에서 `agent_servers.thePrometheus Codex.command`에 `CODEX_HOME`을 붙여주고, 필요하면 `env`로 추가하세요. |
